...
export const UpdateStopDialog = ({ open, onClose, refresh, stop }) => {
  ...
  const checkModification = (newParada) => {
    const modified = Object.keys(initialParada).some(key => initialParada[key] !== newParada[key]);
  ...
  const handleUpdate = async () => {
    ...
    try {
      const response = await driversEndpoints.updateDriversStop(parada._id, parada);
      refresh();
      handleClose();
  ...
  return (
    ...
    >
      ...
      <DialogActions sx={{mx: 2, mb: 1}}>
        ...
        <Button onClick={handleUpdate} disabled={!isModified} variant="contained">Actualizar</Button>
      </DialogActions>
    </Dialog>
  );
};